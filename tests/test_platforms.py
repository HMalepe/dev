...
def test_youtube_dry_run_short_circuits(settings):
    ...
    content = PostContent(topic="t", platform="youtube", text="hello", media_path="clip.mp4")
...
def test_tiktok_dry_run_short_circuits(settings):
    platform = TikTokPlatform(settings)
    content = PostContent(topic="t", platform="tiktok", text="hello", media_path="clip.mp4")
...
def test_tiktok_reports_missing_credentials_when_not_dry_run(settings, monkeypatch):
    monkeypatch.setenv("PIPELINE_DRY_RUN", "false")
    from ai_social_pipeline.config import Settings
